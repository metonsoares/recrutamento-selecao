...
import {
  CalendarX, Search, CheckCircle2, ExternalLink, Download, AlertTriangle,
...
export function FaltasClient({
  ...
  const [situacao, setSituacao] = useState<Situacao>('com_falta')
  const [menuExportar, setMenuExportar] = useState(false)

  const filtradas = linhas.filter(l => {
    if (situacao === 'com_falta' && l.dias === 0) return false
    if (situacao === 'sem_falta' && l.dias > 0) return false
  ...
  const comFalta = linhas.filter(l => l.dias > 0)
  const totalDias = filtradas.reduce((s, l) => s + l.dias, 0)
  const nomeEmpresa = empresas.find(e => e.id === empresaFiltro)?.nome
  const baseNome = `faltas-${competencia.slice(0, 7)}${nomeEmpresa ? '-' + nomeEmpresa.replace(/[^\w]+/g, '-') : ''}`

  const CABECALHO = ['Colaborador', 'Empresa', 'Cargo', 'Faltas (dias)', 'Registros', 'Datas']
  ...
  async function exportarPdf() {
    ...
    const blob = await gerarPdfTabela({
      ...
      subtitulo: `${rotuloMes(competencia)} · ${nomeEmpresa ?? 'Todas as empresas'} · ${filtradas.length} colaboradores · ${totalDias} dia(s)`,
      cabecalho: CABECALHO,
      linhas: corpo(),
      paisagem: true,
    })
    ...
    <div className="p-4 sm:p-6 max-w-6xl mx-auto space-y-5">
      ...
      <div className="flex items-start gap-3 flex-wrap">
        ...
        <div className="flex-1 min-w-[220px]">
          <h1 className="text-2xl font-bold leading-tight">Faltas registradas</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        ...
        <select value={situacao} onChange={e => setSituacao(e.target.value as Situacao)}
          className="h-9 w-full border border-gray-300 rounded-md px-2.5 text-sm bg-white">
          <option value="com_falta">Só quem teve falta</option>
          <option value="sem_falta">Só quem não teve falta</option>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
        ...
        <Cartao titulo="Com falta no mês" valor={String(comFalta.length)} cor="text-red-600" />
        <Cartao titulo="Dias de falta" valor={String(totalDias)} cor="text-amber-600" />
      </div>
          ...
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b">
              <tr className="text-left text-[11px] uppercase tracking-wide text-muted-foreground">
                ...
                <th className="px-4 py-2.5 font-semibold hidden lg:table-cell">Datas</th>
                <th className="px-4 py-2.5" />
              </tr>
            </thead>
            <tbody className="divide-y">
              {filtradas.map(l => (
                <tr key={l.candidate_id} className={`hover:bg-gray-50 ${l.dias > 0 ? 'bg-red-50/30' : ''}`}>
                    ...
                    {l.dias > 0 ? (
                      <>
                        <span className="inline-flex items-center gap-1 text-[11px] font-bold uppercase tracking-wide rounded-full px-2 py-0.5 bg-red-100 text-red-700">
                          ...
                          {l.dias} falta{l.dias !== 1 ? 's' : ''}
                        </span>
                        <span className="block text-[11px] text-muted-foreground mt-0.5">
                          {l.registros} registro{l.registros !== 1 ? 's' : ''}
                          {l.tipos.length > 0 && ` · ${l.tipos.map(t => TIPO_LABEL[t] ?? t).join(', ')}`}
                        </span>
                      </>
                    ) : (
                      <span className="inline-flex items-center gap-1 text-[11px] font-bold uppercase tracking-wide rounded-full px-2 py-0.5 bg-emerald-100 text-emerald-700">
                        <CheckCircle2 className="w-3 h-3" />Sem falta
                      </span>
                    )}
                  </td>
                  <td className="px-4 py-2.5 text-gray-600 hidden lg:table-cell">
                    {l.datas.length > 0 ? l.datas.map(dataCurta).join(', ') : '—'}
                  </td>
                  <td className="px-4 py-2.5 text-right">
                    <Link href={`/admin/candidatos/${l.candidate_id}?tab=ferias`}
              ...
              {filtradas.length === 0 && (
                <tr><td colSpan={5} className="px-4 py-10 text-center text-muted-foreground">
                  {situacao === 'com_falta'
                    ? `Nenhuma falta registrada em ${rotuloMes(competencia).toLowerCase()}.`
