...
import { FreelancersTable } from './freelancers-table'
...
export default async function FreelancersPage() {
  ...
  // Apenas candidatos com status 'freelancer' (coluna Freelancer do Kanban)
  ...
  const tableRows = freelancers.map(c => {
    const app = getApp(c)
    return {
      id: c.id,
    ...
    <div className="p-4 sm:p-6 max-w-5xl mx-auto space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            <h1 className="text-xl font-bold text-gray-900">Freelancers</h1>
            <p className="text-sm text-muted-foreground">
              {freelancers.length} candidato{freelancers.length !== 1 ? 's' : ''} freelancer{freelancers.length !== 1 ? 's' : ''}
            </p>
          </div>
        </div>
      </div>
      ...
      {freelancers.length === 0 && (
        <div className="flex flex-col items-center justify-center py-20 text-center gap-4">
          ...
          <div>
            <p className="font-medium text-gray-600">Nenhum freelancer ainda</p>
            <p className="text-sm text-muted-foreground mt-1">
              Candidatos com status &ldquo;Freelancer&rdquo; aparecem aqui.
            </p>
          </div>
          <Link
            href="/admin/candidatos"
