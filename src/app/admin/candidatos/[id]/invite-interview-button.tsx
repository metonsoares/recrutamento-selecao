...
export function InviteInterviewButton({ candidateId }: Props) {
  ...
  async function handleSend() {
    ...
    if (!confirm('Enviar convite por WhatsApp para o número cadastrado pelo candidato?')) return
    setSending(true)
    try {
  ...
  return (
    <button
      onClick={handleSend}
      disabled={sending}
      className="shrink-0 inline-flex items-center gap-1.5 text-sm font-medium border rounded-lg px-3 py-1.5 transition-colors disabled:opacity-60 border-[#25D366] text-[#128C7E] hover:bg-[#25D366]/10"
