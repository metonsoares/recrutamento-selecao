...
export function TemplatesManager({ initialTemplates, companyOptions }: Props) {
  ...
  function openModal() { setName(''); setEmpresa(''); setFile(null); setError(''); setModalOpen(true) }

  async function handleSave() {
    setError('')
    if (!name.trim()) { setError('Informe o nome do template.'); return }
    if (!file) { setError('Selecione um arquivo (.docx ou .pdf).'); return }
    if (file.size > 15 * 1024 * 1024) { setError('Arquivo excede 15 MB.'); return }
    setSaving(true)
    const fd = new FormData()
    fd.append('file', file); fd.append('name', name); fd.append('empresa', empresa)
    try {
        ...
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md">
            <div className="flex items-center justify-between px-5 py-4 border-b">
              <h2 className="text-base font-semibold text-gray-900">Adicionar template de contrato</h2>
            ...
            <div className="px-5 py-4 space-y-3">
              ...
              <div className="space-y-1">
                <label className="text-xs font-medium text-gray-600">Arquivo * (.docx ou .pdf — até 15 MB)</label>
                {file ? (
