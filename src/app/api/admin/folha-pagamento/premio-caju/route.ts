...
export async function DELETE(req: NextRequest) {
  try {
    ...
    if (!restantes || restantes.length === 0) {
      await supabase.from('premio_caju_ciclos').delete().eq('id', ciclo.id)
      return NextResponse.json({ ok: true, ciclo_removido: true })
    }
    ...
    console.error('[premio-caju DELETE]', err)
    return NextResponse.json({ error: 'Erro interno.' }, { status: 500 })
  }
}
